...
///  12. Finger number 
///---------------------------------------------------------------------------------///
#define FINGER_NUMBER 				(10)		///< 設定手指數，若有開TPINFO，則以TPINFO為主
...
#define X_MAX	 				(1024)		///< the X resolution of TP AA(Action Area)
#define Y_MAX 					(600)		///< the Y resolution of TP AA(Action Area)
...
static u32 zet6221_ts_interrupt(struct zet622x_tsdrv *ts_drv)
{
...
static int __devinit zet622x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ic_model = MODEL_ZET6231; ///< Set the default model name
	...
	if (!int_handle) 
        {
		pr_info( "[ZET] : zet6221_ts_probe.request_irq failed. err %d=\n",int_handle);
...
static int __init zet622x_module_init(void)
{
	...
	if(!ctp_get_system_config())
        {
		printk("[ZET] : %s:read config fail!\n",__func__);
		return ret;
	}

//	ctp_ops.ts_reset();gaidaima
	ctp_reset();
	ctp_wakeup(0,20);
	
	zet622x_i2c_driver.detect = ctp_detect;
	...
	if(ret)
	{	
		printk(KERN_ERR "[ZET] : %s:register chrdev failed\n",__FILE__);	
		return ret;
	}
